# Brownfield Agent — intakeflow

**Run ID:** n97ce7qqajnafd261scnm6xjm983rby9
**Project ID:** jd7eymxp7yzz1q88m4ey68cr0183q4db
**Repo:** /Users/damonbodine/Factory512/builds/intakeflow-20260326-223057/intakeflow
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Smart Priority Scorer — when client completes intake, analyze all intake form responses (housing, employment, health, dependents, urgency) and generate priority score 1-100 with detailed factor breakdown explaining which factors contributed most. Show on client detail page. 2) Needs Assessment Generator — from intake data + screening results, generate structured needs assessment: Presenting Needs, Strengths {{REQUEST}} Resources, Barriers to Service, Recommended Programs, Immediate Safety Concerns. 3) Waitlist Optimization Advisor — when program slots open, analyze waitlist and recommend which clients to enroll based on priority score, wait time, program fit. Show as recommendation panel. 4) Client Communication Drafter — generate personalized status update communications for clients on waitlists with estimated wait time and interim resources. Each feature needs: Convex action, React component, integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/intakeflow-20260326-223057/intakeflow. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n97ce7qqajnafd261scnm6xjm983rby9"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n97ce7qqajnafd261scnm6xjm983rby9"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n97ce7qqajnafd261scnm6xjm983rby9","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n97ce7qqajnafd261scnm6xjm983rby9","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n97ce7qqajnafd261scnm6xjm983rby9","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/intakeflow-20260326-223057/intakeflow`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Smart Priority Scorer — when client completes intake, analyze all intake form responses (housing, employment, health, dependents, urgency) and generate priority score 1-100 with detailed factor breakdown explaining which factors contributed most. Show on client detail page. 2) Needs Assessment Generator — from intake data + screening results, generate structured needs assessment: Presenting Needs, Strengths {{REQUEST}} Resources, Barriers to Service, Recommended Programs, Immediate Safety Concerns. 3) Waitlist Optimization Advisor — when program slots open, analyze waitlist and recommend which clients to enroll based on priority score, wait time, program fit. Show as recommendation panel. 4) Client Communication Drafter — generate personalized status update communications for clients on waitlists with estimated wait time and interim resources. Each feature needs: Convex action, React component, integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n97ce7qqajnafd261scnm6xjm983rby9",
  "projectName": "intakeflow",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Smart Priority Scorer — when client completes intake, analyze all intake form responses (housing, employment, health, dependents, urgency) and generate priority score 1-100 with detailed factor breakdown explaining which factors contributed most. Show on client detail page. 2) Needs Assessment Generator — from intake data + screening results, generate structured needs assessment: Presenting Needs, Strengths {{REQUEST}} Resources, Barriers to Service, Recommended Programs, Immediate Safety Concerns. 3) Waitlist Optimization Advisor — when program slots open, analyze waitlist and recommend which clients to enroll based on priority score, wait time, program fit. Show as recommendation panel. 4) Client Communication Drafter — generate personalized status update communications for clients on waitlists with estimated wait time and interim resources. Each feature needs: Convex action, React component, integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/intakeflow-20260326-223057/intakeflow`
